...
The whole thing is one HTML file: CSS, a little vanilla JavaScript, and the three onboarding photos embedded inline as data URIs, so you can open it from a server, straight off disk, or anywhere you move it. Motion is CSS only and honors reduced motion, and there is no build step and nothing to install. Two things load from the network when it is there, the brand webfonts and the WebGL globe, and both degrade gracefully (system fonts and a static CSS globe), so the experience holds even with no connection.
