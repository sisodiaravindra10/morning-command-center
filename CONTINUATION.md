...
> **Pivot remainder:** (a) **docs as readable HTML pages (#72)** — the docs/01..09 are still `.md`; convert to simple styled HTML pages, a substantial mechanical pass, good next unit; (b) **re-record the films** — they still show the old periwinkle app and can't be re-rendered from this environment cleanly (the user, or a dedicated render setup, should redo them); the film posters on the landing are thus stale.
