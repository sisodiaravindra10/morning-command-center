...
- **#65 Status bar transparent in onboarding** (pairs with light mode, do together): right now onboarding starts at top:48px (a dark canvas strip with the status bar). The user wants the photo full-bleed with a TRANSPARENT status bar floating over it. Cleanest fix: move `.statusbar` out of `.app` to a `.device` child, `position:absolute;top:0;z-index:110;transparent`; give `.app` `padding-top:48px`; revert `#onboarding` and `.regionview` to `inset:0` (full-bleed); move `#rv-back` to `top:54px`.
- **#57 Light mode + toggle/tour top-right**: add `[data-theme="light"]` token override (keep emerald accent), and move the dark/light toggle AND the Take-the-tour control to the top-right of the screen. Default dark. (NOTE: a `#theme-seg` signal/pulse toggle exists in CSS/JS but its UI is not rendered; the new light toggle is a separate `data-theme` axis.)
