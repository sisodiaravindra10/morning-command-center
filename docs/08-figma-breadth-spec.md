...
| `line` | `rgba(174,194,246,0.12)` | 1px periwinkle-tinted hairlines, never heavier |
...
Periwinkle is calm: handled, confident, the AI, anything interactive. Warm sand is you: urgent, going quiet. Each is a small dot, a thin rule, or a tinted chip, never a full card fill.

| Variable | Value | Meaning |
|---|---|---|
| `signal` | `#AEC2F6` | confident, handled, the AI, the one primary action; glows on dark |
| `signal-hi` | `#CBD6FB` | the brighter periwinkle, for hover and text that lifts off a tint |
| `signal-ink` | `#0B0C12` | the dark text that sits on a periwinkle fill |
| `info` | `#AEC2F6` | the thinking and learn periwinkle (same hue as signal) |
...
2. The trust ledger, stated before anything asks for attention: "47 handled overnight, 2 need you, 1 corrected", where "2 need you" is a periwinkle `signal-hi` button that scrolls to the triage. This is the quiet-by-default proof.
3. The altitude toggle, a segmented control "My deals · 4" and "My team · 3". Altitude is a quiet badge here, not a separate destination.
4. The "Needs your judgment" section label (mono, sentence case) with a "one at a time" mini-note.
5. The calm triage: one focus card plus a collapsed queue, ordered by severity. On a calm morning two items need judgment, the deal-risk nudge (the Crestview deal has gone quiet four days, $635K Westlake, the wave glyph reading "62% sure a soft nudge helps", with "Send the nudge" and "Change") and the voice-required outbox (three messages drafted but needing Dana's actual voice, "Open outbox" and "Approve all"). The list is built to about three to five items, not to fill the screen.
6. The guardrails resting state, a calm all-clear button: a good-state dot, "Guardrails: all clear", and a mono sub "security · fair-housing · sync · handoff". This is the restraint made visible on a quiet morning.
7. The FYI, a no-action item: a good-state tick, "Bennett pack ready to sign, no action needed", routing to the closing-pack sheet. Resolved, not a decision.
8. The dashboard glance (collapsed by default, expands on tap): the head reads "$14.2M pipeline · 22 live · on track"; expanded it shows the pipeline hero numeral ($14.2M, up 8% week), a KPI grid (2 need you, 8.5h AI saved overnight, 94% SLA met, 1 at risk), and a day rail (now, the 9:00 Oak St call, the 11:00 Lakeside showing, the 16:00 Bennett close). The early-warning context built into the home, not a separate feature.
9. The "Review activity" section label, then the handled ledger: a `good` check tick, "47 handled overnight, 8.5h saved", a chevron that expands to four near-miss rows (fresh comps for 3 listings, 12 portal enquiries in SLA, 4 viewings booked, 6 documents chased) and "43 more, view full log". Each row carries a tappable "why" provenance stamp. Build both the collapsed and expanded states.
...
3. Source-of-truth reconciler (amber dot): "Three systems disagree on the Marsh deal's status." MLS pending, CRM active, dotloop has an unsigned addendum, the wave glyph reading "78% sure dotloop is the truth". Actions "Trust dotloop, fix all 3" and "Review".
...
Never, collected so the restraint is impossible to miss: no colored side rules on cards, no full-color card fills, no gradient text, no glass on content surfaces, and washes capped at the listed opacities (the periwinkle and good washes at 0.12, the danger wash at 0.14). State lives in the dot, not the card.
...
The honest alternative to a precision chip. An inline row, a small animated wave glyph (four or five bars) plus a `sans/body` line in Leo's voice naming how sure he is, for example "78% sure dotloop is the truth" or "62% sure a soft nudge helps". The prototype carries confidence this way, never as a fixed "fairly sure" pill and never as a bare percentage on a chip. There is no "very sure" state and no low-as-red chip; an unsure item routes to Dana's eyes instead.
...
Four tabs: Today, Day, Team, Leads, in a floating liquid-glass capsule (30px radius, translucent with a heavy backdrop blur, a bright top edge, and a soft drop shadow). A periwinkle glass pill glides under the active tab. Build the pill as a position variant `active` = today / day / team / leads so each breadth frame shows the correct tab lit (active = periwinkle label and icon over the glass pill; idle = ink-subtle). Today and Team carry a small badge count. Four tabs, not five or more, so the bar stays calm and one-handed. The AI states legend and Orchestration sit off the tab bar; in the live prototype the guided tour steps to each of them in turn.
...
- `orb` (a static component; in the prototype this maps to Dana's avatar, which carries a static periwinkle glow, not a breathing animation. The live idle motions are the thinking dot, the confidence waves, the day-spine "now" node, and the healthy deal-pulse lines).
...
5. The remaining bespoke components: the range bar, the undo ring, the pulse sparkline, the leo-says block, the bottom nav, a floating glass capsule with a gliding active pill, the cover and resting chips, the sheet template.
