...
| `signal-ink` | `#0B0C12` | the dark text that sits on an emerald fill |
...
There is no separate text variant: on dark the accents are already light enough to read as small text, and where text needs to lift further it uses `signal-hi #46DD9B`. Text on an emerald-filled element is `signal-ink #0B0C12`, not white.

Washes and lines (the only fills and borders the accents are allowed):
`signal-wash rgba(21,192,122,0.12)` with `signal-line rgba(21,192,122,0.32)`, `good-wash rgba(121,184,156,0.14)`, `danger-wash rgba(231,199,156,0.14)` with `danger-line rgba(231,199,156,0.42)`, `info-wash rgba(21,192,122,0.12)`.
...
`r 8` (chips, buttons, segmented controls, small controls), `r-md 13` (cards, the glance, deck blocks), `r-lg 18` (the hero and larger deck blocks), `r-paper 14` (the read band). Build each as a variable and bind it. Do not type radii by hand per layer. One intentional one-off radius sits outside this scale: the bottom-sheet corner (22). Leave that as a literal pixel value, not a bound token. The bottom nav is a floating, rounded liquid-glass capsule (30px radius, translucent with a heavy backdrop blur and a bright top edge) carrying an emerald glass pill that glides under the active tab.

### Type, the three voices
Set up three text-style families. Voice is load-bearing: it tells the reader who is speaking.

- Display, Host Grotesk (held in a legacy token literally named `--serif`; no actual serif loads). Leo's display voice and the greeting: the greeting, the read-band paper copy, the drafted message Leo speaks in the voice outbox, and the big pipeline numeral. This is the character of the product.
- Interface, sans, Inter. The lines Leo speaks on a card, body, labels, buttons, navigation, list titles, descriptions, sheet titles.
- Machine, mono, JetBrains Mono. Section labels, row metadata, counts, and source traces. Sentence case (never all-caps), around 8.5 to 11px for labels and meta, tracking +0.12em by default, the section labels a touch wider at +0.14em.

| Style name | Font | Size / line | Weight | Notes |
|---|---|---|---|---|
| `display/pipeline-num` | Host Grotesk | 37 banner / 40 region / 1 | 600 | the pipeline numeral ($14.2M) in the globe banner, the biggest number in the app; -0.02em. The "100+" in orchestration is a small mono cap, not a display numeral |
...
1. App bar. The greeting "Good morning, Dana" in `display/title-xl` with a mono sub "Sat 27 Jun", a pause-all glyph (the blast-radius control, badged 14), a download-team-report button, and Dana's avatar. Human voice first, on purpose.
2. The trust ledger, stated before anything asks for attention: "47 handled overnight, 2 need you, 1 corrected", where "2 need you" is an emerald `signal-hi` button that scrolls to the triage. This is the quiet-by-default proof.
3. The globe banner, the calm visual header the morning opens on, filling roughly the top 40% under the appbar. On the right, a faded, slowly rotating dark world tinted to the emerald, white markers on the metros, the sphere bleeding off the right edge. On the left, Leo's live portfolio read in three lines: a "Leo, on watch overnight" label beside a breathing orb, the pipeline numeral "$14.2M" large, and "pipeline, 22 live across 6 metros", plus an "explore the map" affordance. This is the geographic, portfolio-wide view, distinct from the ledger's three-count tally above it. Draw the static globe state for Figma; annotate that in the prototype the live WebGL globe is a progressive enhancement layered over this static fallback and that the banner is pinned while the triage sheet scrolls over it (section 5).
4. The altitude toggle, a glass segmented control "My deals · 4" and "My team · 3" sitting just below the globe banner: a rounded pill with a translucent blurred ground and an emerald indicator pill that glides under the active segment (the active label reads over the pill, the other settles to a quieter ink). Altitude is a quiet badge here, not a separate destination.
...
9. The dashboard glance (collapsed by default, expands on tap): the head reads "22 live · on track"; expanded it shows a KPI grid (2 need you, 8.5h AI saved overnight, 94% SLA met, 1 at risk) and a day rail (now, the 9:00 Oak St call, the 11:00 Lakeside showing, the 16:00 Bennett close). The pipeline hero numeral ($14.2M) lives in the globe banner above, not here. The early-warning context built into the home, not a separate feature.
...
### Frame 10, Day, the spine
The Day tab, the second tab in the bottom nav. A vertical day timeline (the spine): a hairline `line` rule down the left with a node per appointment, past ticks muted, the "now" node a `signal` dot that pulses a soft emerald ring, and upcoming ticks ahead of it. Each tick carries a mono time, an Inter event title, and a tappable prep card naming what the agents prepped for it (the 9:00 Oak St call, the 11:00 Lakeside showing, the 16:00 Bennett close). A "now" flag rides the current tick. Each prep card opens its day-brief sheet. This is the same day content the glance day rail previews, given its own full-screen tab.
...
Never, collected so the restraint is impossible to miss: no colored side rules on cards, no full-color card fills, no gradient text, no glass on content surfaces, and washes capped at the listed opacities (the emerald wash at 0.12, the good and danger washes at 0.14). State lives in the dot, not the card.
...
Four tabs: Today, Day, Team, Leads, in a floating liquid-glass capsule (30px radius, translucent with a heavy backdrop blur, a bright top edge, and a soft drop shadow). An emerald glass pill glides under the active tab. Build the pill as a position variant `active` = today / day / team / leads so each breadth frame shows the correct tab lit (active = emerald label and icon over the glass pill; idle = ink-subtle). Today and Team carry a small badge count. Four tabs, not five or more, so the bar stays calm and one-handed. The AI states legend and Orchestration sit off the tab bar; in the live prototype the guided tour steps to each of them in turn.

### Globe banner, `component: globe-banner`
The calm visual header of Today, roughly the top 40% under the appbar. A faded, slowly rotating dark world tinted to the emerald on the right, white metro markers, the sphere bleeding off the right edge; on the left, Leo's live portfolio read in three lines (a "Leo, on watch overnight" label and breathing orb, the "$14.2M" pipeline numeral in `display/pipeline-num`, and "pipeline, 22 live across 6 metros"), plus an "explore the map" affordance. Build the static globe as the Figma component. Note on the handoff that the prototype layers a live WebGL globe (the `cobe` library) over this static fallback as a progressive enhancement, that no content depends on the globe rendering, and that the banner is pinned while the triage sheet scrolls over it (section 5). It is the geographic, portfolio-wide read, distinct from the trust ledger's three-count tally above.

### Altitude toggle, `component: altitude`
The glass segmented control on Today, two segments "My deals · 4" and "My team · 3" in a rounded glass pill (translucent with a backdrop blur, a bright top edge), `r` (8). An emerald indicator pill glides under the active segment; build it as a position variant `active` = my-deals / my-team (active = the label reads over the pill; idle = `ink-muted`). It shares the floating-glass-plus-gliding-pill language of the bottom nav, kept to the small controls that float, never on content surfaces. Altitude is a badge on one ranked list, not a separate destination.

### Buttons, `component: btn`
Variants: `kind` = primary (`btn-amber`, an emerald `signal` fill, `signal-ink` dark label, a soft emerald glow) / ghost (transparent, `ink-muted` text, hairline border) / danger (a `danger` fill, for the defensive security action and pause-all only). `width` = full / sm (for the two-up action rows). `r` (8). The rule from the system: exactly one filled primary action per screen, everything else ghost. State `pressed` = scale 0.985 (note it; Figma cannot animate it, but the handoff should say so).
