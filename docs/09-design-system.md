...
6. **Anti-slop.** No all-caps labels, no blurred shapes behind text, no colored side-stripes, no gradient text, no fake numeric precision, no em-dashes. See section 11.
...
| JetBrains Mono | `--mono` | The machine layer: section labels, row metadata, counts, confidence tags, the small print. Sentence case, tracked out. |
...
|---|---|---|---|---|---|---|
| Pipeline numeral | Host Grotesk | 37 banner / 40 region / 1 | 600 | -0.02em | ink | the $14.2M pipeline figure in the dashboard glance hero (the biggest number in the app, in the Host Grotesk display family) |
| Greeting | Host Grotesk | 19 / 1.2 | 600 | -0.02em | ink | the morning greeting, "Good morning, Dana" |
...
Rules of thumb: Host Grotesk on the greeting, the paper read-band, the drafted lines Leo speaks, and the big pipeline numeral, never on running interface text. The big pipeline numeral is Host Grotesk, the display family, not mono. Mono only on short labels and counts, never on a sentence. **Sentence case everywhere; never all-caps** (decorative caps read as AI-slop). Tighten tracking as display type gets larger; leave body and mono at their natural spacing.

## 3. Color

Dark premium. A near-black canvas with an emerald accent, and a warm sand reserved for what needs a human.
...
| `--good-wash` `rgba(121,184,156,0.14)` | a one-off `rgba(121,184,156,0.4)` border on done chips |
...
Rule: text on an emerald-filled element (the primary button) is `--signal-ink` `#0B0C12`, not white. Washes are for backgrounds and focus bands only.
...
- **Screen switching:** there is no push or slide-from-the-right. Switching screens toggles an `.is-active` class that swaps `display` and plays a short `screenIn` fade-and-rise in place (`translateY(6px)` plus opacity). The bottom nav carries an emerald glass pill that glides under the active tab as you switch; the active tab also recolors its label to `--signal-hi` and shows its badge count. The scroll position resets to the top on each switch.
...
| Uncertain | an emerald dot with an expanding ring | surfaces its own doubt and two options, a range not a point, and what it could not check |
| Confident | an emerald check that draws itself | "Done, here is what changed", always with an 8-second undo |
...
- **Altitude toggle:** the My deals / My team switch on Today, just below the globe banner. A glass segmented control, a rounded pill with a blurred translucent ground, carrying an emerald indicator pill that glides between the two segments on `transform`; the active label reads over the pill, the other settles to a quieter ink. It shares the floating-glass-plus-gliding-pill treatment of the bottom nav, reserved for the small controls that float, never on content surfaces. Altitude is a badge on one ranked list, not a separate destination.
- **Bottom nav** (`.tabbar`): four roots (Today, Day, Team, Leads), as a floating liquid-glass capsule (30px radius, `backdrop-filter: blur(26px) saturate(185%)`) so the content refracts through it, with a bright top edge and a soft drop shadow. An emerald glass pill glides under the active tab; the active label picks up `--signal-hi`, and Today and Team carry a badge count. Content scrolls beneath the floating bar.
...
- **Pause / blast-radius sheet** (opened via `#pause-btn` / `sheetOpen('pause')`): the pause-all control opens a sheet that spells out exactly what goes quiet (in-flight actions held, client replies paused) versus what stays on for safety (confirmed showings still auto-confirm), then offers a duration choice (an hour, until a set time, or until you resume) that live-updates the resume note and the action button, with an auto-resume chip.
