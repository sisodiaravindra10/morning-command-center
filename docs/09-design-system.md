...
The system makes Leo's state legible at all times, and there is a dedicated legend screen (`data-screen="legend"`, reached from the presenter jump nav or the orchestration glyph) that names all five in Leo's own voice.
...
There is one theme and no light or color-mode toggle. A presenter "Theme" segmented control sets `data-mode` on the root, between two density modes that share the same palette and type families.

- **Signal (default, calm):** the decision surface. The dashboard glance is collapsed, the KPI grid shows two cells, and the per-card pulse sparklines are hidden. Attention is spent only on what needs a human.
- **Pulse (stats-dense):** the same screen turned up for numbers. The glance auto-expands, the KPI grid widens to four cells with sparklines, and the pulse glyphs appear. Same tokens, same type families, more on screen.
