...
- **Screen switching:** there is no push or slide-from-the-right. Switching screens toggles an `.is-active` class that swaps `display` and plays a short `screenIn` fade-and-rise in place (`translateY(6px)` plus opacity). The bottom nav carries a periwinkle glass pill that glides under the active tab as you switch; the active tab also recolors its label to `--signal-hi` and shows its badge count. The scroll position resets to the top on each switch.
...
- **Globe banner:** the calm visual header that opens Today, roughly its top 40% under the appbar. A faded, slowly rotating dark world tinted to the periwinkle (a real WebGL globe via the `cobe` library) bleeds off the right edge with periwinkle metro markers; alongside it on the left is Leo's live portfolio read, a "Leo, on watch overnight" label and breathing orb, the "$14.2M" pipeline numeral, "pipeline, 22 live across 6 metros", a periwinkle ticker line, and "100+ agents active, as of 6:52am". The geographic, portfolio-wide read, distinct from the trust ledger's tally in the appbar. The live globe is a progressive enhancement: a static globe sits behind it, so the banner reads correctly even if WebGL or the module is unavailable, and no content depends on the globe rendering. The banner is pinned while the triage sheet scrolls up over it.
...
- **Altitude toggle:** the My deals / My team switch on Today, just below the globe banner. A glass segmented control, a rounded pill with a blurred translucent ground, carrying a periwinkle indicator pill that glides between the two segments on `transform`; the active label reads over the pill, the other settles to a quieter ink. It shares the floating-glass-plus-gliding-pill treatment of the bottom nav, reserved for the small controls that float, never on content surfaces. Altitude is a badge on one ranked list, not a separate destination.
- **Bottom nav** (`.tabbar`): four roots (Today, Day, Team, Leads), as a floating liquid-glass capsule (30px radius, `backdrop-filter: blur(26px) saturate(185%)`) so the content refracts through it, with a bright top edge and a soft drop shadow. A periwinkle glass pill glides under the active tab; the active label picks up `--signal-hi`, and Today and Team carry a badge count. Content scrolls beneath the floating bar.
